...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private var plotx: Double? = null
    private var ploty: Double? = null
    ...
    private fun setUpMap(googleMap: GoogleMap) {
        ...
        mMap.setOnMapLongClickListener {
            latLng -> mMap.addMarker(MarkerOptions().position(latLng)
                .title("lat: "+latLng.latitude.toString() + "long: "+latLng.longitude.toString()))
            if (check!!>0) drawLine(latLng.latitude,latLng.longitude)
            plotx = latLng.latitude
            ploty = latLng.longitude
            check = check!!+1
        }
    ...
    private  fun drawLine(lat:Double,long:Double) {
